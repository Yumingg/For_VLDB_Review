...
%data = player_10;
%%%%%%%%%%%%%%%%%%%%%%5player
% load('5player_geometrical.mat')
load('5player_arithmetic.mat')
x = [1 100 1000 9999];
data = player_stake;
[player_num, cases, block] = size(data);
stake = zeros(player_num,4);
for i = 1 : 4
    for player = 1 : player_num-1
        stake(player_num-player,i) = mean(data(player,:,x(i)));
    end
end
for i = 1 : 4
        stake(player_num,i) = mean(data(player_num,:,x(i)));
end
%%%%%%%%%%%%%%%%%%%%%%5player
% 5 Players
...
for k1 = 1:length(b)
    ...
    Ysum = Ysum + ydt(k1,:);
    text(ctr(k1,:)-0.1, Ysum - ydt(k1,:).*0.85, sprintfc('%.2f', ydt(k1,:)), 'HorizontalAlignment','center', 'VerticalAlignment','bottom','Fontname', 'Times New Roman','FontSize',15,'BackgroundColor',b(k1).FaceColor)
end
